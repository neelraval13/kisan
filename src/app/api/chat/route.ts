...
export async function POST(req: NextRequest) {
  ...
  if (!userId || (!message?.trim() && !imageFile && !audioFile)) {
    console.log('Validation failed - need userId and either message or files');
    return NextResponse.json(
      { error: 'Missing required fields: userId and either message or files' },
      { status: 400 }
    );
  }

  console.log('Validation passed:', { userId, message: message?.trim(), hasImage: !!imageFile, hasAudio: !!audioFile });

    console.log('Received:', { userId, message, hasImage: !!imageFile, hasAudio: !!audioFile });
    ...
    // TODO: Handle image and audio files if needed
    // For now, we'll just process the text message
...
${summary ? `\n### Conversation Summary\n${summary}` : ''}`;
    ...
    try {
      // Try to get formData for error translation
      const formData = await req.formData();
      const message = formData.get('message') as string;
